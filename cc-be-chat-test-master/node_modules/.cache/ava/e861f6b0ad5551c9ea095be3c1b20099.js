...
(0, _ava.default)("checkWords test", t => {
  t.is(profantiyfilter.checkWords("nobhead"), '******');
}); // 玩家ID索引

var uid = 1; // 创建 WebSocket 服务
...
wss.on('connection', function connection(ws) {
  ...
  ws.on('message', function incoming(message) {
    if (message == 'client request connect') {
      ...
      var player = new Player(uid, "玩家名字", ws); // 添加玩家

      playermgr.addPlayer(uid, player); // 同步聊天记录
      ...
      playermgr.getPlayerByTask(ws, function (palyer) {
        // 检查屏蔽字
        var wordafter = profantiyfilter.checkWords(palyer.playerInfo() + message); // 广播聊天记录

        playermgr.broadCasts(wordafter); // 添加聊天记录

        chatrecords.addChatRecord(wordafter);
        var wordaftermessage = profantiyfilter.checkWords(message); // 解析GM命令

        gmmgr.parse(wordaftermessage, palyer, popularmgr); // 添加流行词
...
profantiyfilter.loadTXT(); // 加载GM命令
...
//# sourceMappingURL=D:\code\cc-be-chat-test-master\node_modules\.cache\ava\e861f6b0ad5551c9ea095be3c1b20099.js.map